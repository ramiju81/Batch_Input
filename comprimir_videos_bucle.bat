...
for /r "%base_folder%" %%F in (*.mp4) do (
    ...
    :loop
    REM Definir ruta de salida temporal
    set "output_video=%%~dpF%%~nF_tmp.mp4"

    REM Comprimir video con calidad controlada
    ffmpeg -i "!current_video!" -vcodec libx264 -crf 23 -preset slow "!output_video!" -y

    REM Verificar si el archivo se redujo
    set "new_size=!prev_size!"
    if exist "!output_video!" set "new_size=!output_video:~z!"

    if !new_size! LSS !prev_size! (
        echo Tamaño reducido: !prev_size! -> !new_size! >> "%log_file%"
        del "!current_video!"
        rename "!output_video!" "%%~nxF"
        set "current_video=%%F"
        set "prev_size=!new_size!"
        set "reduced=1"
        goto loop
    )

    if !reduced! EQU 0 (
        echo No se pudo reducir más el archivo: "%%F" >> "%log_file%"
        del "!output_video!"
    )
)
